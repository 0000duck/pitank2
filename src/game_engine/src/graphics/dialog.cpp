...
Dialog::Dialog(int argc, char **argv, QWidget *parent) : QDialog(parent), ui(new Ui::Dialog), qnode(argc, argv) {
  ...
  timer_sim = clock();
  //pthread_t p_thread;
  //pthread_create(&p_thread, NULL, (THREADFUNCPTR) &Dialog::reset_timer, this);
...
void Dialog::initialize_pitank_sim(const gazebo_msgs::ModelStates& msg) {
  ...
  for(int i=0; i<robotn_sim; i++){
      ...
      //t = new tag(msg.pose[id+1],robot_z[id],team_id,gameStart,aiGame,controls_pub[0]);
  ...
  if(gameStart == true && robot.size() > 0) {
    if(robot[0].autonomous_drive == false && robot[0].immobilized == false && pause_flag == false) {
      switch(robot[0].collisionStateVar) {
        ...
        case 0:
          //if(simGame) robot[0].vel1 = -LINEAR_SPEED;
          robot[0].vel1 = -127/6;
          robot[0].vel2 = 0;
        break;
      ...
      /*if(robot[0].collisionStateVar == -1 && speed.angular.z == SHOOT && bullet_interval[0].nsecsElapsed() > nsecs_between_bullets && game == 0) {
    ...
    //here
    if(simGame){
        /*if(robot[0].collisionFlag){
            twist[0].linear.x = robot[0].vel1;
            twist[0].angular.z = robot[0].vel2;
            controls_pub[0].publish(twist[0]);
        }
        else if(robot[0].autonomous_drive){
            twist[0].linear.x = (robot[0].vel1/17)*LINEAR_SPEED;
            twist[0].angular.z = (robot[0].vel2/50)*ANGULAR_SPEED;
            controls_pub[0].publish(twist[0]);
        }*/
  ...
  if(gameStart == true && robot.size() > 1) {
      if(robot[1].autonomous_drive == false && robot[1].immobilized == false && pause_flag == false) {
        switch(robot[1].collisionStateVar) {
          ...
          case 0:
            //if(simGame) robot[1].vel1 = -LINEAR_SPEED;
            robot[1].vel1 = -127/6;
            robot[1].vel2 = 0;
          break;
        ...
        /*if(robot[1].collisionStateVar == -1 && speed.angular.z == SHOOT && bullet_interval[0].nsecsElapsed() > nsecs_between_bullets && game == 0) {
      ...
      //here
      /*if(simGame){
          if(robot[1].collisionFlag){
              twist[1].linear.x = robot[1].vel1;
              twist[1].angular.z = robot[1].vel2;
              controls_pub[1].publish(twist[1]);
          }
          else if(robot[1].autonomous_drive){
              twist[1].linear.x = (robot[1].vel1/17)*LINEAR_SPEED;
              twist[1].angular.z = (robot[1].vel2/50)*ANGULAR_SPEED;
              controls_pub[1].publish(twist[1]);
          }
      }*/
      if(simGame){
  ...
  if(gameStart == true && robot.size() > 2) {
      if(robot[2].autonomous_drive == false && robot[2].immobilized == false && pause_flag == false) {
        switch(robot[2].collisionStateVar) {
          ...
          case 0:
            if(simGame) robot[2].vel1 = -LINEAR_SPEED;
            else robot[2].vel1 = -127/6;
            robot[2].vel2 = 0;
          break;
        ...
        /*if(robot[2].collisionStateVar == -1 && speed.angular.z == SHOOT && bullet_interval[0].nsecsElapsed() > nsecs_between_bullets && game == 0) {
      ...
      //here
      if(simGame){
          if(robot[2].collisionFlag){
              twist[2].linear.x = robot[2].vel1;
              twist[2].angular.z = robot[2].vel2;
              controls_pub[2].publish(twist[2]);
          }
          else if(robot[2].autonomous_drive){
              twist[2].linear.x = (robot[2].vel1/17)*LINEAR_SPEED;
              twist[2].angular.z = (robot[2].vel2/50)*ANGULAR_SPEED;
              controls_pub[2].publish(twist[2]);
          }
      }
      else{
  ...
  if(gameStart == true && robot.size() > 3) {
      if(robot[3].autonomous_drive == false && robot[3].immobilized == false && pause_flag == false) {
        switch(robot[3].collisionStateVar) {
          ...
          case 0:
            if(simGame) robot[3].vel1 = -LINEAR_SPEED;
            else robot[3].vel1 = -127/6;
            robot[3].vel2 = 0;
          break;
        ...
        /*if(robot[3].collisionStateVar == -1 && speed.angular.z == SHOOT && bullet_interval[0].nsecsElapsed() > nsecs_between_bullets && game == 0) {
      ...
      //here
      if(simGame){
          if(robot[3].collisionFlag){
              twist[3].linear.x = robot[3].vel1;
              twist[3].angular.z = robot[3].vel2;
              controls_pub[3].publish(twist[3]);
          }
          else if(robot[3].autonomous_drive){
              twist[3].linear.x = (robot[3].vel1/17)*LINEAR_SPEED;
              twist[3].angular.z = (robot[3].vel2/50)*ANGULAR_SPEED;
              controls_pub[3].publish(twist[3]);
          }
      }
      else{
