        ...
        switch (state_gotoxy){
            case ROTATE:
                if(sqrt((xf-robot[id].x)*(xf-robot[id].x)+(yf-robot[id].y)*(yf-robot[id].y)) < ERRO_DIST){
                    ...
                    move_left(id);
                    //printf("rodar esquerda:   %f\n", sign);
                }
                else if(sign >= 0){
                    move_right(id);
                    //printf("rodar direita:  %f\n", sign);
                }
            break;
            case TRANSL:
...
void move_back(int id){
    //vel[id].linear.x = -float(LINEAR_SPEED/10);
    vel[id].linear.x = -SPEED_BITS;
    vel[id].angular.z = 0;
    vel_pub[id].publish(vel[id]);
    ros::spinOnce();
...
void move_left(int id){
    vel[id].linear.x = 0;
    //vel[id].angular.z = ANGULAR_SPEED;
...
/*void idle(){
    if(uistate.aiGame){
        if(uistate.simGame){
            for (int id=0;id<robot.size();id++){
                vel[id].linear.x = (robot[id].vel1/17)*LINEAR_SPEED;
                vel[id].angular.z = (robot[id].vel2/50)*ANGULAR_SPEED;
                vel_pub[id].publish(vel[id]);
            }
        }
    }
...
void testDestroyWall(int id){
    if(robot[id].angle > 80 && robot[id].angle < 100 && (flag_test>0)){
        stop(id);
    }
    else move_left(id);
    if(robot[id].angle < -70 && robot[id].angle > -110){
        flag_test++;
        stop(id);
    }
    printf("%d\n",flag_test);
...
static bool isValid(int x, int y){
    /*if((x-70-offset+SPACE) % SPACE == 0 && (y-offset+SPACE) % SPACE == 0){
        if(map_astar[(x-70-offset+SPACE)/SPACE-1][(y-offset+SPACE)/SPACE-1]){
            return true;
        }
        return false;
    }
...
void moveToRandomPosition(int id){
    ...
    //printf("antes de isvalid, x= %d      y= %d\n",x,y);
    if(isValid(x,y) && !flag){
        //printf("dentro de isvalid\n");
        Node player;
...
bool doIntersect(QPointF *p1, QPointF *q1)
{
    ...
    //printf("SIZE: %lu", w_array.wall.size());
...
void decisionMaking(){
    //moveToRandomPosition(0);
    //moveToBestAttackPosition(0,1);
    //moveToBestDefensePosition(0,1);
    //findBestWalls(1);
    //GoToXY(0,gridXtoPixel(13),gridYtoPixel(3));
    //testDestroyWall(0);
                        /*QPointF* p1 = convertToGrid(robot[0].x, robot[0].y);
                        QPointF* p2 = convertToGrid(robot[1].x, robot[1].y);
                        player.x = p1->x();
                        player.y = p1->y();
                        enemy.x = p2->x();
                        enemy.y = p2->y();
                        //negaMax3(DEPTH_NEGAMAX,0,player,enemy);
                        negaMax2(DEPTH_NEGAMAX,0,player,enemy,player,0);
...
void testecenas(Node player, Node dest, int flag_teste){
    if(flag_teste == 0){
        std::vector<Node> n = aStar(player,dest);
        printNodes(n);
        followPath(n);
        flag_test = 1;
    }
...
int main(int argc, char **argv){
        ...
        /*std::vector<Node> n = aStar(player,dest1);
        followPath(n);
